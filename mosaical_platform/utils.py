                    ...
                    try:
                        ...
                        if active_loan.current_debt > 0:
                            ...
                            if remaining_yield > 0:
                                ...
                                profile.vbtc_balance += remaining_yield
                                profile.save()
                                ...
                                Transaction.objects.create(
                                    ...
                                    related_nft=nft,
                                    description=f'Excess yield {remaining_yield:.8f} vBTC to balance'
                                )
                        else:
                            # No debt, yield goes directly to user
                            profile = UserProfile.objects.get(user=nft.owner)
                            profile.vbtc_balance += total_yield
                            profile.save()
                            ...
                            Transaction.objects.create(
                                ...
                                amount=total_yield,
                                related_nft=nft,
                                description=f'Yield {total_yield:.8f} vBTC from {nft.collection.name} #{nft.token_id}'
                            )
